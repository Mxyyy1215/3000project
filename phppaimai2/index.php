...
<div class="hotlist">
    ...
    <ul>
	  ...
	  $sql="select * from tb_study where recommend=1 and zt=1 order by eaid desc limit 0,3 "; //查询10条数据库内容
	$rs=mysqli_query($link,$sql);
	while($row=mysqli_fetch_array($rs)){
		?>
      <li>
...
<div class="indexright">
  ...
  <div class="hotkecheng">
    <ul>
	  ...
	  $sql="select * from tb_study where   zt=1 order by eaid desc limit 0,6 "; //查询10条数据库内容
	$rs=mysqli_query($link,$sql);
	while($row=mysqli_fetch_array($rs)){
		?>
      <li>
